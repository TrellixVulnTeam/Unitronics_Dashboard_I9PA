...
r"""
.. dialect:: ***REMOVED***+psycopg2
    :name: psycopg2
    :dbapi: psycopg2
    :connectstring: ***REMOVED***+psycopg2://user:password@host:port/dbname[?key=value&key=value...]
...
.. tip::
    ...
    that they are passed as explicit keyword arguments to :func:`_sa.create_engine()`::
        ...
        engine = create_engine(
            "***REMOVED***+psycopg2://scott:tiger@localhost/test",
...
connection URI::
    ...
    engine = sa.create_engine(
        "***REMOVED***+psycopg2://scott:tiger@192.168.0.199:5432/test?sslmode=require"
    )
...
which specifies Unix-domain communication rather than TCP/IP communication::
    ...
    create_engine("***REMOVED***+psycopg2://user:password@/dbname")
...
using ``host`` as an additional keyword argument::
    ...
    create_engine("***REMOVED***+psycopg2://user:password@/dbname?host=/var/lib/***REMOVED***")

.. seealso::

    `PQconnectdbParams \
    <https://www.***REMOVED***.org/docs/current/static/libpq-connect.html#LIBPQ-PQCONNECTDBPARAMS>`_
...
information provided to make the connections::
    ...
    create_engine(
        "***REMOVED***+psycopg2://user:password@/dbname?host=HostA:port1&host=HostB&host=HostC"
    )
...
.. seealso::
    ...
    `PQConnString \
    <https://www.***REMOVED***.org/docs/current/libpq-connect.html#LIBPQ-CONNSTRING>`_
...
initial scheme::
    ...
    engine = create_engine('***REMOVED***+psycopg2://')
...
.. seealso::
    ...
    <https://www.***REMOVED***.org/docs/current/libpq-envars.html>`_ -
...
which may be passed to :func:`_sa.create_engine`::
    ...
    engine = create_engine(
        "***REMOVED***+psycopg2://scott:tiger@host/dbname",
...
* For PostgreSQL 9.1 and above, the ``client_encoding`` parameter may be
  ...
  ``libpq`` PostgreSQL client library::
    ...
    engine = create_engine("***REMOVED***+psycopg2://user:pass@host/dbname?client_encoding=utf8")
  ...
  ``libpq``::
    ...
    engine = create_engine(
        "***REMOVED***+psycopg2://user:pass@host/dbname",
...
* A common way to set up client encoding with PostgreSQL databases is to
  ensure it is configured within the server-side ***REMOVED***.conf file;
...
As discussed in :ref:`***REMOVED***_isolation_level`,
...
via the ``sqlalchemy.dialects.***REMOVED***`` logger.  When this logger
is set to the ``logging.INFO`` level, notice messages will be logged::

    import logging

    logging.getLogger('sqlalchemy.dialects.***REMOVED***').setLevel(logging.INFO)
...
follows::
    ...
    engine = create_engine("***REMOVED***+psycopg2://scott:tiger@localhost/test",
...
logger = logging.getLogger("sqlalchemy.dialects.***REMOVED***")
