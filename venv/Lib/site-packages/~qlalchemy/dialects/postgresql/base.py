...
To set isolation level using :func:`_sa.create_engine`::
    ...
    engine = create_engine(
        "***REMOVED***+pg8000://scott:tiger@localhost/test",
        execution_options={
...
.. seealso::
    ...
    :ref:`***REMOVED***_readonly_deferrable`
...
.. _***REMOVED***_readonly_deferrable:
...
independently of the isolation level by passing the ``***REMOVED***_readonly`` and
``***REMOVED***_deferrable`` flags with
...
"READ ONLY" and "DEFERRABLE"::
    ...
    with engine.connect() as conn:
        conn = conn.execution_options(
            ...
            ***REMOVED***_readonly=True,
            ***REMOVED***_deferrable=True
        )
        with conn.begin():
            #  ... work with transaction
...
.. versionadded:: 1.4 added support for the ``***REMOVED***_readonly``
   and ``***REMOVED***_deferrable`` execution options.

.. _***REMOVED***_alternate_search_path:
...
:ref:`***REMOVED***_schema_reflection`, SQLAlchemy is generally organized around
...
at :ref:`schema_set_default_connections`::
    ...
    engine = create_engine("***REMOVED***+psycopg2://scott:tiger@host/dbname")
...
.. admonition:: Section Best Practices Summarized
    ...
    ``***REMOVED***_ignore_search_path`` option will cause all reflected
...
<https://www.***REMOVED***.org/docs/current/static/ddl-schemas.html#DDL-SCHEMAS-PATH>`_.
...
reflection process as follows::
    ...
    >>> engine = create_engine("***REMOVED***://scott:tiger@localhost/test")
    >>> with engine.connect() as conn:
...
``***REMOVED***_ignore_search_path`` option, which can be specified as a
...
:meth:`_schema.MetaData.reflect`::
    ...
    ...     referring = Table('referring', metadata_obj,
    ...                       autoload_with=conn,
    ...                       ***REMOVED***_ignore_search_path=True)
    ...
...
:func:`_***REMOVED***.insert()` function, which provides
the generative methods :meth:`_***REMOVED***.Insert.on_conflict_do_update`
and :meth:`~.***REMOVED***.Insert.on_conflict_do_nothing`:

.. sourcecode:: pycon+sql

    >>> from sqlalchemy.dialects.***REMOVED*** import insert
...
.. seealso::
    ...
    `INSERT .. ON CONFLICT
    <https://www.***REMOVED***.org/docs/current/static/sql-insert.html#SQL-ON-CONFLICT>`_
...
* The :paramref:`_***REMOVED***.Insert.on_conflict_do_update.index_elements` argument
...
* When using :paramref:`_***REMOVED***.Insert.on_conflict_do_update.index_elements` to
  infer an index, a partial index can be inferred by also specifying the
  use the :paramref:`_***REMOVED***.Insert.on_conflict_do_update.index_where` parameter:
...
* The :paramref:`_***REMOVED***.Insert.on_conflict_do_update.constraint` argument is
...
* The :paramref:`_***REMOVED***.Insert.on_conflict_do_update.constraint` argument may
...
:paramref:`_***REMOVED***.Insert.on_conflict_do_update.set_` parameter.  This
...
.. warning::
    ...
    :paramref:`_***REMOVED***.Insert.on_conflict_do_update.set_` dictionary.

Updating using the Excluded INSERT Values
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

In order to refer to the proposed insertion row, the special alias
:attr:`~.***REMOVED***.Insert.excluded` is available as an attribute on
the :class:`_***REMOVED***.Insert` object; this object is a
...
a WHERE clause using the :paramref:`_***REMOVED***.Insert.on_conflict_do_update.where`
...
:meth:`~.***REMOVED***.Insert.on_conflict_do_nothing` method:
...
The :class:`_***REMOVED***.TSVECTOR` type can provide for explicit CAST::

    from sqlalchemy.dialects.***REMOVED*** import TSVECTOR
...
.. tip::
    ...
    regarding
    `Full Text Search <https://www.***REMOVED***.org/docs/current/textsearch-controls.html>`_.
...
search strategies, the ``match`` method accepts a ``***REMOVED***_regconfig``
keyword argument::

    select(mytable.c.id).where(
        mytable.c.title.match('somestring', ***REMOVED***_regconfig='english')
    )
...
``to_tsvector()`` command as the initial argument::
    ...
    select(mytable.c.id).where(
            ...
            .match('somestring', ***REMOVED***_regconfig='english')
        )
...
.. seealso::
    ...
    `Full Text Search <https://www.***REMOVED***.org/docs/current/textsearch-controls.html>`_ - in the PostgreSQL documentation
...
syntaxes. It uses SQLAlchemy's hints mechanism::
    ...
    # SELECT ... FROM ONLY ...
    result = table.select().with_hint(table, 'ONLY', '***REMOVED***')
    print(result.fetchall())

    # UPDATE ONLY ...
    table.update(values=dict(foo='bar')).with_hint('ONLY',
                                                   dialect_name='***REMOVED***')

    # DELETE FROM ONLY ...
    table.delete().with_hint('ONLY', dialect_name='***REMOVED***')
...
The ``***REMOVED***_include`` option renders INCLUDE(colname) for the given
string names::

    Index("my_index", table.c.x, ***REMOVED***_include=['y'])
...
using the ``***REMOVED***_where`` keyword argument::

  Index('my_index', my_table.c.id, ***REMOVED***_where=my_table.c.value > 10)

.. _***REMOVED***_operator_classes:
...
an index (see
https://www.***REMOVED***.org/docs/current/interactive/indexes-opclass.html).
The :class:`.Index` construct allows these to be specified via the
``***REMOVED***_ops`` keyword argument::
...
Note that the keys in the ``***REMOVED***_ops`` dictionaries are the
...
If ``***REMOVED***_ops`` is to be used against a complex SQL expression such
...
:class:`_***REMOVED***.ExcludeConstraint` construct using the
:paramref:`_***REMOVED***.ExcludeConstraint.ops` parameter. See that parameter for
details.

.. versionadded:: 1.3.21 added support for operator classes with
   :class:`_***REMOVED***.ExcludeConstraint`.
...
https://www.***REMOVED***.org/docs/current/static/indexes-types.html). These can be
specified on :class:`.Index` using the ``***REMOVED***_using`` keyword argument::

    Index('my_index', my_table.c.data, ***REMOVED***_using='gin')

The value passed to the keyword argument will be simply passed through to the
underlying CREATE INDEX command, so it *must* be a valid index type for your
version of PostgreSQL.

.. _***REMOVED***_index_storage:
...
parameters can be specified on :class:`.Index` using the ``***REMOVED***_with``
keyword argument::

    Index('my_index', my_table.c.data, ***REMOVED***_with={"fillfactor": 50})

.. versionadded:: 1.0.6

PostgreSQL allows to define the tablespace in which to create the index.
The tablespace can be specified on :class:`.Index` using the
``***REMOVED***_tablespace`` keyword argument::

    Index('my_index', my_table.c.data, ***REMOVED***_tablespace='my_tablespace')

.. versionadded:: 1.1

Note that the same option is available on :class:`_schema.Table` as well.

.. _***REMOVED***_index_concurrently:

Indexes with CONCURRENTLY
^^^^^^^^^^^^^^^^^^^^^^^^^

The PostgreSQL index option CONCURRENTLY is supported by passing the
flag ``***REMOVED***_concurrently`` to the :class:`.Index` construct::

    tbl = Table('testtbl', m, Column('data', Integer))

    idx1 = Index('test_idx1', tbl.c.data, ***REMOVED***_concurrently=True)
...
construct, the DBAPI's "autocommit" mode must be used::
    ...
    table = Table(
        ...
        Column("id", String))
    index = Index(
        "foo_idx", table.c.id, ***REMOVED***_concurrently=True)
...
.. seealso::
    ...
    :ref:`***REMOVED***_isolation_level`

.. _***REMOVED***_index_reflection:
...
of :class:`.PGInspector`, which offers additional methods::
    ...
    engine = create_engine("***REMOVED***+psycopg2://localhost/test")
...
* ``TABLESPACE``::
    ...
    Table("some_table", metadata, ..., ***REMOVED***_tablespace='some_tablespace')

  The above option is also available on the :class:`.Index` construct.

* ``ON COMMIT``::

    Table("some_table", metadata, ..., ***REMOVED***_on_commit='PRESERVE ROWS')

* ``WITH OIDS``::

    Table("some_table", metadata, ..., ***REMOVED***_with_oids=True)

* ``WITHOUT OIDS``::

    Table("some_table", metadata, ..., ***REMOVED***_with_oids=False)

* ``INHERITS``::

    Table("some_table", metadata, ..., ***REMOVED***_inherits="some_supertable")

    Table("some_table", metadata, ..., ***REMOVED***_inherits=("t1", "t2", ...))
...
* ``PARTITION BY``::
    ...
    Table("some_table", metadata, ...,
          ***REMOVED***_partition_by='LIST (part_column)')
...
.. seealso::
    ...
    <https://www.***REMOVED***.org/docs/current/static/sql-createtable.html>`_ -
    in the PostgreSQL documentation.

.. _***REMOVED***_constraint_options:
...
* ``NOT VALID``:  This option applies towards CHECK and FOREIGN KEY constraints
  ...
  that renders ALTER TABLE constructs, the ``***REMOVED***_not_valid`` argument
  ...
  these constructs from the migration operation directives::
       ...
       CheckConstraint("some_field IS NOT NULL", ***REMOVED***_not_valid=True)

       ForeignKeyConstraint(["some_id"], ["some_table.some_id"], ***REMOVED***_not_valid=True)
  ...
  .. seealso::
      ...
      <https://www.***REMOVED***.org/docs/current/static/sql-altertable.html>`_ -
      in the PostgreSQL documentation.

.. _***REMOVED***_table_valued_overview:
...
* ``unnest()`` - in order to generate a PostgreSQL ARRAY literal, the
  :func:`_***REMOVED***.array` construct may be used::


    >>> from sqlalchemy.dialects.***REMOVED*** import array
    >>> from sqlalchemy import select, func
...
.. seealso::
    ...
    <https://www.***REMOVED***.org/docs/current/sql-expressions.html#SQL-SYNTAX-ROW-CONSTRUCTORS>`_

    `PostgreSQL Row Constructor Comparison
    <https://www.***REMOVED***.org/docs/current/functions-comparisons.html#ROW-WISE-COMPARISON>`_
...
* :class:`_***REMOVED***.ARRAY` - ARRAY datatype

* :class:`_***REMOVED***.array` - array literal

* :func:`_***REMOVED***.array_agg` - ARRAY_AGG SQL function

* :class:`_***REMOVED***.aggregate_order_by` - helper for PG's ORDER BY aggregate
  function syntax.
...
* :class:`_***REMOVED***.JSON`

* :class:`_***REMOVED***.JSONB`
...
* :class:`_***REMOVED***.HSTORE` - HSTORE datatype

* :class:`_***REMOVED***.hstore` - hstore literal
...
* :class:`_***REMOVED***.ENUM` - DDL and typing support for ENUM.

* :meth:`.PGInspector.get_enums` - retrieve a listing of current ENUM types

* :meth:`.***REMOVED***.ENUM.create` , :meth:`.***REMOVED***.ENUM.drop` - individual
  CREATE and DROP commands for ENUM.

.. _***REMOVED***_array_of_enum:
...
class TSVECTOR(sqltypes.TypeEngine):
    ...
    """The :class:`_***REMOVED***.TSVECTOR` type implements the PostgreSQL
...
class ENUM(sqltypes.NativeForEmulated, sqltypes.Enum):
    ...
    True, the PostgreSQL backend will use a :class:`_***REMOVED***.ENUM`
    ...
    When using :class:`_types.Enum` or :class:`_***REMOVED***.ENUM`
    ...
    :class:`_***REMOVED***.ENUM` independently, and associate it with the
    ...
    .. versionchanged:: 1.0.0 The PostgreSQL :class:`_***REMOVED***.ENUM` type
    ...
    def __init__(self, *enums, **kw):
        """Construct an :class:`_***REMOVED***.ENUM`.
        ...
        :param create_type: Defaults to True.
         ...
         :meth:`~.***REMOVED***.ENUM.create`
         or :meth:`~.***REMOVED***.ENUM.drop`
         are called directly.
         Setting to ``False`` is helpful
         when invoking a creation scheme to a SQL file
         without access to the actual database -
         the :meth:`~.***REMOVED***.ENUM.create` and
         :meth:`~.***REMOVED***.ENUM.drop` methods can
        ...
        if native_enum is False:
            util.warn(
                ...
                "sqlalchemy.dialects.***REMOVED***.ENUM datatype; this type "
    ...
    def adapt_emulated_to_native(cls, impl, **kw):
        """Produce a PostgreSQL native :class:`_***REMOVED***.ENUM` from plain
        :class:`.Enum`.
...
class PGCompiler(compiler.SQLCompiler):
    ...
    def visit_match_op_binary(self, binary, operator, **kw):
        if "***REMOVED***_regconfig" in binary.modifiers:
            regconfig = self.render_literal_value(
                binary.modifiers["***REMOVED***_regconfig"], sqltypes.STRINGTYPE
            )
            if regconfig:
...
class PGDDLCompiler(compiler.DDLCompiler):
    ...
    def _define_constraint_validity(self, constraint):
        not_valid = constraint.dialect_options["***REMOVED***"]["not_valid"]
    ...
    def visit_create_index(self, create):
        ...
        if self.dialect._supports_create_index_concurrently:
            concurrently = index.dialect_options["***REMOVED***"]["concurrently"]
            if concurrently:
                text += "CONCURRENTLY "
        ...
        using = index.dialect_options["***REMOVED***"]["using"]
        if using:
            text += (
                "USING %s "
        ...
        ops = index.dialect_options["***REMOVED***"]["ops"]
        text += "(%s)" % (
            ", ".join(
                [
        ...
        includeclause = index.dialect_options["***REMOVED***"]["include"]
        if includeclause:
            inclusions = [
                index.table.c[col]
        ...
        withclause = index.dialect_options["***REMOVED***"]["with"]
        if withclause:
            text += " WITH (%s)" % (
                ", ".join(
                    [
        ...
        tablespace_name = index.dialect_options["***REMOVED***"]["tablespace"]
        if tablespace_name:
            text += " TABLESPACE %s" % preparer.quote(tablespace_name)

        whereclause = index.dialect_options["***REMOVED***"]["where"]
    ...
    def post_create_table(self, table):
        table_opts = []
        pg_opts = table.dialect_options["***REMOVED***"]
    ...
    def get_insert_default(self, column):
        if column.primary_key and column is column.table._autoincrement_column:
            if column.server_default and column.server_default.has_argument:
                ...
                try:
                    seq_name = column._***REMOVED***_seq_name
                    ...
                    column._***REMOVED***_seq_name = seq_name = name
...
class PGDialect(default.DefaultDialect):
    ...
    connection_characteristics = connection_characteristics.union(
        {
            "***REMOVED***_readonly": PGReadOnlyConnectionCharacteristic(),
            "***REMOVED***_deferrable": PGDeferrableConnectionCharacteristic(),
        }
    )
    ...
    reflection_options = ("***REMOVED***_ignore_search_path",)
    ...
    def initialize(self, connection):
        ...
        # https://www.***REMOVED***.org/docs/9.3/static/release-9-2.html#AEN116689
    ...
    def get_foreign_keys(
        ...
        table_name,
        schema=None,
        ***REMOVED***_ignore_search_path=False,
        **kw
    ):
        ...
        FK_SQL = """
          ...
          ORDER BY 1
        """
        # https://www.***REMOVED***.org/docs/9.0/static/sql-createtable.html
        FK_REGEX = re.compile(
        ...
        for conname, condef, conschema in c.fetchall():
            ...
            if ***REMOVED***_ignore_search_path:
    ...
    def _pg_index_any(self, col, compare_to):
        if self.server_version_info < (8, 1):
            # https://www.***REMOVED***.org/message-id/10279.1124395722@sss.pgh.pa.us
    ...
    def get_indexes(self, connection, table_name, schema, **kw):
        ...
        # returned as a list by some drivers such as py***REMOVED***
        ...
        for row in c.fetchall():
            ...
            if not has_idx:
                ...
                # to have a ``***REMOVED***_using`` in it that is just the
                # default, so for the moment leaving this out.
                if amname and amname != "btree":
                    index["amname"] = amname

                if filter_definition:
                    index["***REMOVED***_where"] = filter_definition
