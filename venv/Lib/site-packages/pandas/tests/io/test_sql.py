...
SQL_STRINGS = {
    "read_parameters": {
        ...
        "***REMOVED***": 'SELECT * FROM iris WHERE "Name"=%s AND "SepalLength"=%s',
    },
    ...
    "read_no_parameters_with_percent": {
        ...
        "***REMOVED***": "SELECT * FROM iris WHERE \"Name\" LIKE '%'",
    },
}
...
def iris_table_metadata(dialect: str):
    ...
    dtype = Float if dialect == "***REMOVED***" else REAL
    metadata = MetaData()
    iris = Table(
        "iris",
        metadata,
...
@pytest.fixture
def ***REMOVED***_psycopg2_engine(iris_path, types_data):
    sqlalchemy = pytest.importorskip("sqlalchemy")
    pytest.importorskip("psycopg2")
    engine = sqlalchemy.create_engine(
        "***REMOVED***+psycopg2://***REMOVED***:***REMOVED***@localhost:5432/pandas"
    )
    check_target = sqlalchemy.inspect(engine) if _gt14() else engine
    if not check_target.has_table("iris"):
        create_and_load_iris(engine, iris_path, "***REMOVED***")
    if not check_target.has_table("types"):
        create_and_load_types(engine, types_data, "***REMOVED***")
    yield engine
    with engine.connect() as conn:
        with conn.begin():
            stmt = sqlalchemy.text("DROP TABLE IF EXISTS test_frame;")
...
@pytest.fixture
def ***REMOVED***_psycopg2_conn(***REMOVED***_psycopg2_engine):
    yield ***REMOVED***_psycopg2_engine.connect()
...
***REMOVED***_connectable = [
    "***REMOVED***_psycopg2_engine",
    "***REMOVED***_psycopg2_conn",
]
...
sqlalchemy_connectable = mysql_connectable + ***REMOVED***_connectable + sqlite_connectable

sqlalchemy_connectable_iris = (
    mysql_connectable + ***REMOVED***_connectable + sqlite_iris_connectable
)
...
@pytest.mark.db
@pytest.mark.parametrize("conn", ***REMOVED***_connectable)
...
class TestSQLApi(SQLAlchemyMixIn, _TestSQLApi):
    ...
    def test_pg8000_sqlalchemy_passthrough_error(self):
        ...
        db_uri = "***REMOVED***+pg8000://user:pass@host/dbname"
...
class _TestSQLAlchemy(SQLAlchemyMixIn, PandasSQLTest):
    ...
    def test_datetime_with_timezone(self):
        # edge case that converts ***REMOVED*** datetime with time zone types
        ...
        # xref #7139
        # this might or might not be converted depending on the ***REMOVED*** driver
        df = sql.read_sql_table("types", self.conn)
        check(df.DateColWithTz)
...
class _TestPostgreSQLAlchemy:
    ...
    def connect(cls):
        return sqlalchemy.create_engine(
            f"***REMOVED***+{cls.driver}://***REMOVED***:***REMOVED***@localhost:{cls.port}/pandas"
        )
    ...
    def test_schema_support(self):
        ...
        # only test this for ***REMOVED*** (schema's not supported in
        # mysql/sqlite)
